...
//1 if the test(C container, List<Test<C>> tests),container not null,the headline is its classname,otherwise,you should setHeadline();
//2 the calculate is growing huge as the  initialize(param.size)s param.size growing
//3 test(C container, List<Test<C>> tests)
public class ListPerformance {
  ...
  static List<Test<TreeSeries>> ts=
    new ArrayList<Test<TreeSeries>>();
  static {
    ts.add(new Test<TreeSeries>("main") {
      @Override
      int test(TreeSeries container, TestParam tp) {
        int loops = tp.loops;
    ...
    tests.add(new Test<List<Integer>>("add") {
      int test(List<Integer> list, TestParam tp) {
        ...
        for(int i = 0; i < loops; i++) {//10, 5000, 100, 5000, 1000, 5000, 10000, 500
          list.clear();
  ...
  static class TreeSeriesTest extends Tester<TreeSeries> {

    public TreeSeriesTest(TreeSeries container, List<Test<TreeSeries>> tests) {
      super(container, tests);
    }

    @Override
    protected TreeSeries initialize(int size) {
      return new TreeSeries();
    }
  }
  public static void main(String[] args) {

//    TreeSeriesTest treeSeriesTest = new TreeSeriesTest(new TreeSeries(), ts);
//    treeSeriesTest.timedTest();
    if(args.length > 0)
